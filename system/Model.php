    ...
    class Model {
        ...
        public function insert($table, $data){
            ...
            foreach ($data as $key => $value){
                $qKeys .= $key.',';
                $qValues .= '"'. $this->db->real_escape_string($value) .'"' .',';
            }
            $qKeys = substr($qKeys,0,-1);
            $qValues = substr($qValues,0,-1);
            $result = $this->db->query("INSERT INTO $table ($qKeys) VALUES ($qValues)");
            return $result;
        }

        public function delete($from, $where = "1"){
            return $this->db->query("DELETE FROM $from WHERE $where");
        }
        ...
        public function update($table, $data, $where = "1"){
            ...
            return $this->db->query("UPDATE $table SET $toSet WHERE $where");
        }
